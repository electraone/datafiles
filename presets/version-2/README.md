...
### Scrollable lists
A long touch (longer than 0.5sec) on a control will show up a window with large version of the control. Lists have been reworked so that they become swipeable containers of list items. We would like to get close to a tablet-like behaviour.
...
A picture is worth of thousand words. Often it is hard to descride a list item with words. For those situations we added a way to provide graphical bitmap data. The idea is at its very beginning, but feel it is an important addition that needs to be developed further.
...
### Dials
Many people asked about these. We are still not convinced if they are realy needed.
...
### Macro
With version 2 it is possible to assign one pot to several controls. In such situation turning the pot will affect values of all assigned controls.
